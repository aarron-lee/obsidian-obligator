...
import { FileSuggest, FolderSuggest, FileSelectionModal } from "./ui";
	...
	async onload() {
		...
		const run_obligator = async () => {
			const run_create_note_logic = async (TEMPLATE_FILE) => {
				...
				const NOTE_FOLDER = this.app.vault.getAbstractFileByPath(
					normalizePath(this.settings.note_path),
				);
				if (NOTE_FOLDER == undefined) {
					new Notice(
						`The note path "${this.settings.note_path}" specified in the settings does not exist, aborting...`,
					);
					return;
				}

				// Make sure the note folder exists
				const TEMPLATES_FOLDER = this.app.vault.getAbstractFileByPath(
					normalizePath(this.settings.templates_folder),
				);
				if (TEMPLATES_FOLDER == undefined) {
					new Notice(
						`The note path "${this.settings.templates_folder}" specified in the settings does not exist, aborting...`,
					);
					return;
				}
				...
				if (TEMPLATE_FILE == undefined) {
					if (["", null].includes(this.settings.template_path)) {
						new Notice(
							`You must specify a template file in the settings.`,
						);
					} else {
						new Notice(
							`The template file "${this.settings.template_path}" specified in the settings does not exist.`,
						);
					}
					return;
				}
				if (!(TEMPLATE_FILE instanceof TFile)) {
					new Notice(
						`${this.settings.template_path} is not a regular file! Aborting.`,
					);
					return;
				}
				...
				let template_contents =
					await this.app.vault.read(TEMPLATE_FILE);

				// Make sure the initial / terminal settings are valid.
				if (
					this.settings.initial === null ||
					this.settings.initial === undefined
				) {
					this.settings.initial = "";
				}
				if (
					this.settings.terminal === null ||
					this.settings.terminal === undefined
				) {
				...
				if (
					...
					const initial_index = check_template_lines.indexOf(
						this.settings.initial,
					);
					const terminal_index = check_template_lines.indexOf(
						this.settings.terminal,
					);
					if (terminal_index <= initial_index) {
						new Notice(
							"The initial heading must preceed the terminal heading. Aborting.",
						);
						return;
					}
				}
				...
				if (this.settings.archive) {
					if (["", null].includes(this.settings.archive_path)) {
						new Notice(
							"The archive path must be specified when the archive option is turned on, aborting.",
						);
						return;
					}
				}
				...
				const ARCHIVE_DATE_FORMAT =
					this.settings.archive_date_format ||
					DEFAULT_SETTINGS.archive_date_format;
				...
				const find_all_notes = (path: string): TFile[] => {
					...
					if (
						...
						for (let child of abstract.children) {
							notes = notes.concat(
								find_all_notes(`${path}/${child.name}`),
							);
						}
					}
					return notes;
				...
				notes.sort((a, b) => {
					const a_name = a.path.slice(
						this.settings.note_path.length + 1,
					);
					const b_name = b.path.slice(
						this.settings.note_path.length + 1,
					);
					return (
						window
							.moment(b_name, this.settings.date_format)
							.valueOf() -
						window
							.moment(a_name, this.settings.date_format)
							.valueOf()
					);
				});
				...
				for (let i = 0; i < notes.length; i++) {
					...
					const sub_path = notes[i].path
						.slice(this.settings.note_path.length + 1)
						.slice(0, -3);
					// The final boolean makes the moment parse in strict mode
					const note_moment = window.moment(
						sub_path,
				...
				if (last_note) {
					const last_note_content =
						await this.app.vault.read(last_note);
					const last_note_lines = strip_frontmatter(
						last_note_content.split("\n"),
					);
					let last_note_initial_index = last_note_lines.indexOf(
						this.settings.initial,
					);
					if (this.settings.initial === "") {
						...
						new Notice(
							`${last_note.basename} does not contain the specified initial heading... aborting.`,
						);
						return;
					}
					let last_note_terminal_index = last_note_lines.indexOf(
						this.settings.terminal,
					);
					if (this.settings.terminal === "") {
						...
						new Notice(
							`${last_note.basename} does not contain the specified terminal heading... aborting.`,
						);
						return;
					}
					last_note_structure = structurize(
						last_note_lines.slice(
							last_note_initial_index,
							last_note_terminal_index,
						),
					);
				}
				...
				for (let i = 0; i < template_lines.length; i++) {
					...
					if (OBLIGATION_REGEX.test(line)) {
						...
						i++;
						let should_trigger = should_trigger_obligation(
							line,
							NOW,
						);
						...
						if (!should_trigger && last_note) {
							...
							const sub_path = last_note.path.slice(
								this.settings.note_path.length + 1,
							);
							let skipped_moment = window.moment(
								sub_path,
								this.settings.date_format,
							);
							while (
								skipped_moment.add(1, "d").isBefore(NOW) &&
								!should_trigger
							) {
								should_trigger = should_trigger_obligation(
									line,
									skipped_moment,
								);
							}
						...
						if (should_trigger) {
							...
							try {
								...
								new Notice(
									`Template malformed, "${line}" must be followed by another line`,
								);
								return;
							}
						}
				...
				let processed_initial_index = processed_lines.indexOf(
					this.settings.initial,
				);
				if (this.settings.initial === "") {
					...
					new Notice(
						`${TEMPLATE_FILE.basename} does not contain the specified initial heading... aborting.`,
					);
					return;
				}
				let processed_terminal_index = processed_lines.indexOf(
					this.settings.terminal,
				);
				if (this.settings.terminal === "") {
					...
					new Notice(
						`${TEMPLATE_FILE.basename} does not contain the specified terminal heading... aborting.`,
					);
					return;
				}
				let template_structure = structurize(
					processed_lines.slice(
						processed_initial_index,
						processed_terminal_index,
					),
				);
				const OUTPUT_INITIAL_LINES = processed_lines.slice(
					0,
					processed_initial_index,
				);
				const OUTPUT_TERMINAL_LINES = processed_lines.slice(
					processed_terminal_index,
				);
				...
				output_file = await this.app.vault.create(
					NEW_NOTE_PATH,
					new_note_lines.join("\n"),
				);
				...
				if (this.settings.archive && last_note) {
					const last_note_name = last_note.path.slice(
						this.settings.note_path.length + 1,
					);
					const last_note_moment = window.moment(
						last_note_name,
						this.settings.date_format,
					);
					const archive_note_name =
						last_note_moment.format(ARCHIVE_DATE_FORMAT);
					const archive_note_path = `${this.settings.archive_path}/${archive_note_name}.md`;
					try {
						const archive_directories = archive_note_path
							.split("/")
							.slice(0, -1);
						for (let i = 1; i <= archive_directories.length; i++) {
							const sub_path = archive_directories
								.slice(0, i)
								.join("/");
							const abstract =
						...
						new Notice(
							`A file called ${archive_note_path} already exists, archival skipped.`,
						);
					}
				}
			};
			...
			new FileSelectionModal(
				this.app,
				this.settings.templates_folder,
				(selectedFile) => {
					run_create_note_logic(selectedFile.path);
				},
			).open();
		};
...
class ObligatorSettingTab extends PluginSettingTab {
	...
	async getHeadings(file_path: string) {
		...
		let headings: { [index: string]: any } = Array.from(
			content.matchAll(HEADING_REGEX_GLOBAL),
	...
	async display(): Promise<void> {
		...
		new Setting(containerEl)
			...
			.addText((text) => {
				...
				text.setValue(this.plugin.settings.note_path).onChange(
					async (value) => {
		...
		new Setting(containerEl)
			...
			.setDesc("Folder to select Templates from")
			.addText((text) => {
				new FolderSuggest(this.app, text.inputEl);
				text.setValue(this.plugin.settings.templates_folder).onChange(
					async (value) => {
		...
		const terminal_value =
			Object.keys(headings).find(
				(key) => headings[key] === this.plugin.settings.terminal,
			) || "";
		const initial_value =
			Object.keys(headings).find(
				(key) => headings[key] === this.plugin.settings.initial,
			) || "";
		// --------------------------------------------------------------------
		...
		new Setting(containerEl)
			...
			.setDesc(
				`(Optional) The heading from the template which begins what Obligator will copy. Everything before this heading in your note will be ignored. If left blank, Obligator will copy from the beginning of the file.`,
			)
		...
		new Setting(containerEl)
			...
			.setDesc(
				`(Optional) The heading from the template which terminates what Obligator will copy. Everything after this heading in your note will be ignored. If left blank, Obligator will copy to the end of the file.`,
			)
			.addDropdown((dropdown) =>
				dropdown
					.addOptions(headings)
		...
		new Setting(containerEl)
			...
			.setDesc(
				`Open your Obligator note automatically when you open this vault.`,
			)
			.addToggle((toggle) => {
				toggle
		...
		new Setting(containerEl)
			...
			.setDesc(
				`Enabling this will move the previous to-do note into the
		...
		const archive_date_format_el =
			setting_archive_date_format.descEl.createEl("b", {
				cls: "u-pop",
				text: "test",
			});
		// @ts-ignore
		...
		new Setting(containerEl)
			...
			.setDesc(
				`If this is enabled, obligator will automatically delete
				...
				toggle
					...
					.onChange(async (value) => {
						...
						if (setting_keep_template_headings instanceof Setting) {
							if (
								toggle_keep_template_headings instanceof
								ToggleComponent
							) {
								setting_keep_template_headings.setDisabled(
									!value,
								);
								this.plugin.settings.keep_template_headings =
									value;
		...
		setting_keep_template_headings = new Setting(containerEl)
			...
			.setDesc(
				`This prevents the setting above from deleting any
		...
		setting_keep_template_headings = new Setting(containerEl)
			...
			.setDesc(
				`To-dos which are children of other to-dos will not be
