...
export default class Obligator extends Plugin {
	...
	async onload() {
		...
		const run_obligator = async () => {
			...
			const TEMPLATE_FILE = this.app.vault.getAbstractFileByPath(
				`${this.settings.template_path}.md`,
			);
			...
			notes.sort((a, b) => {
				...
				return (
					window.moment(b_name, this.settings.date_format).valueOf() -
...
class ObligatorSettingTab extends PluginSettingTab {
	...
	async display(): Promise<void> {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"New daily notes will utilize the template file specified.",
			)
			.addText((text) => {
				new FileSuggest(this.app, text.inputEl);
				text.setValue(this.plugin.settings.template_path).onChange(
					async (value) => {
